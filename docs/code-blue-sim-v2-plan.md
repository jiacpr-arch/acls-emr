...
### 7.1 Character art pipeline — รูป ชื่อ และท่าแอ็คชั่นของตัวละคร

**หลักการสำคัญ: engine ไม่ผูกกับสไตล์ภาพ** — ในเกม ตัวละครคือแค่ `{ charId, pose }`
→ ไฟล์ภาพที่ `public/characters/{charId}/{pose}.webp` เปลี่ยนภาพได้โดยไม่แตะโค้ดเกมเลย
(mockup ปัจจุบันใช้ SVG วาดด้วยโค้ดเป็น placeholder — ใช้งานจริงได้ทันทีระหว่างรอภาพจริง)

**1) Character bible (เขียนเองได้เหมือนเขียนโจทย์)** — นิยามตัวละครเป็นข้อมูล:
...
```jsonc
{
  ...
  "name": "พยาบาลมิ้นท์",
  "role": "Nurse / IV & Drug",
  "personality": "มือไวใจนิ่ง ขานยาเสียงดังฟังชัด",
  "appearance": "หญิงไทยวัย 28 ผมมวยต่ำ ชุด scrub เขียวมิ้นท์ ป้ายชื่อ MorRoo",
  "poses": ["idle", "talk", "panic", "stern", "happy", "action_inject", "action_point"]
}
```

**2) ชุดท่า (pose set) มาตรฐานต่อตัวละคร** — bust แบบ AA + ท่าแอ็คชั่นประจำบทบาท:

| Pose | ใช้เมื่อ | ตัวอย่างท่าแอ็คชั่นประจำตัว |
|---|---|---|
| idle / talk / panic / stern / happy | บทสนทนาทั่วไป (5 ท่าหลัก) | — |
| action ประจำบทบาท 1-2 ท่า | จังหวะ intervention | พี่บอย: ท่ากดหน้าอก · หมอฝน: ชูแผ่น paddle "CLEAR!" · มิ้นท์: ดันยาเข้า IV · อ.เดช: ชี้นิ้วแบบ "ค้าน!" |

รวม ~6-7 ภาพ/ตัวละคร × 5 ตัวละคร ≈ 30-35 ภาพ (ทำทีละตัวได้ ไม่ต้องครบก่อนเปิดใช้)

**3) วิธีผลิตภาพ — 3 ทาง (เลือกหรือผสมได้)**

| วิธี | ค่าใช้จ่าย | ข้อดี/ข้อเสีย |
|---|---|---|
| **A. AI-generate** (แนะนำเริ่มก่อน) | ~ฟรี | เร็ว ทำ 30 ภาพได้ในวันเดียว — ต้องคุม consistency: สร้าง "reference sheet" ของแต่ละตัวก่อน แล้ว generate ทุกท่าโดยแนบภาพอ้างอิง + prompt template เดิม (สไตล์: "flat anime, Ace Attorney-inspired, bust portrait, transparent background") |
| **B. จ้างนักวาด** | ~500-2,000฿/ตัว (ราคาตลาดไทยต่อ character sheet) | คุณภาพ+consistency ดีที่สุด เหมาะทำหลัง gameplay นิ่งแล้ว — ส่ง character bible + ภาพ AI เป็น brief ได้เลย |
| **C. SVG โค้ด (ของ mockup)** | ฟรี | ใช้ได้วันนี้ สลับสีหน้า/ขยับปากด้วยโค้ด แต่สไตล์จำกัด — เก็บเป็น fallback |

**4) Spec ไฟล์ภาพ**
- PNG โปร่งใส → บีบเป็น WebP กว้าง ~600px (บนจอแสดง ~275px, รองรับจอ retina)
- ขยับปากตอนพูด: generate 2 เฟรมต่อท่า (ปากปิด/ปากอ้า) สลับด้วย JS แบบเดียวกับ mockup — หรือใช้ bob animation เฉยๆ ก็ได้อารมณ์ AA แล้ว
- ชื่อไฟล์: `public/characters/nurse_mint/panic.webp`, `nurse_mint/panic_talk.webp`

**5) ผูกกับระบบเขียนโจทย์** — คลังตัวละครเป็นของกลาง ใช้ร่วมกันทุกโจทย์:
โจทย์อ้างแค่ `"who": "nurse_mint", "mood": "panic"` — คนเขียนโจทย์เลือกตัวละคร/สีหน้าจาก dropdown ใน editor ไม่ต้องยุ่งกับไฟล์ภาพ และเพิ่มตัวละครใหม่ = เพิ่มโฟลเดอร์ภาพ + entry ใน character bible เท่านั้น
