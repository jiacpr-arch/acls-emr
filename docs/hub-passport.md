...
## กล่องบัญชี JIA + ออกจากระบบทุกที่ — 24 ก.ย. 2569

- `src/components/precourse/JiaAccountCard.jsx` อยู่บนหน้าบทเรียน (`/pre-course`) และหน้าใบประกาศ — ไม่ต้องหาปุ่มในหน้าต่าง "ระบุตัวผู้เรียน" อีก:
  ยังไม่ login = ปุ่ม "เข้าสู่ระบบด้วยบัญชี JIA"; login แล้ว = ชื่อ + เลขบัตร, ปุ่ม "ใช้บัญชีนี้กับผู้เรียนในเครื่องนี้" (ถ้ายังไม่ผูก) และ "ออกจากระบบ";
  deployment ที่ไม่ได้ตั้ง `HUB_PASSPORT_*` ไม่แสดงอะไร
- "ออกจากระบบ" (และ "ไม่ใช่ฉัน" ในหน้าต่างระบุตัว) = **ออกทุกที่**: `POST /api/passport/logout {everywhere:true, returnTo}` ลบ cookie
  ของแอป แล้วตอบ `hubLogoutUrl` → browser ไป `class.jiacpr.com/sso/logout` (jia-learning-hub) ที่ออกจาก session ของ Hub แล้วพากลับมา
  `returnTo` — คนถัดไปบนเครื่องเดียวกันจะต้อง login ใหม่ ไม่ถูกพาเข้าบัญชีเดิม. `POST` แบบไม่มี body (bundle เก่า) ยังลบแค่ cookie ของแอปเหมือนเดิม
