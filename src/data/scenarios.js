...
export const scenarios = [
...

  {
    ...
    steps: [
      ...
      },
      {
        ...
        hint_th: 'CT พบเลือดออก → บันทึกผล CT = Hemorrhagic (ห้ามให้ tPA เด็ดขาด — absolute contraindication)',
      },
      {
        trigger: 'after_ct',
        scenario_th: 'ยืนยัน Hemorrhagic stroke → ไม่ให้ tPA/antiplatelet\nReverse anticoagulation (ถ้ามี) + เป้า SBP ~140 (Nicardipine drip)\nปรึกษา Neurosurgery ด่วน + ยกหัวเตียง 30°',
        correctActions: ['monitor'],
        hint_th: 'ทำ ICH management checklist ให้ครบ → Done → Monitor GCS ใกล้ชิด (ระวัง herniation)',
      },
    ],
  },

  {
    ...
    steps: [
      ...
      {
        trigger: 'after_fast',
        scenario_th: 'FAST: พูดสับสน + แขนขวาอ่อนแรงเล็กน้อย\nทุกรายที่สงสัย stroke → ต้องเช็ค DTX ก่อนเสมอ!',
        correctActions: ['check_dtx'],
        hint_th: 'ไปหน้า Initial Treatment → บันทึก DTX (จะพบว่า = 42 mg/dL → Hypoglycemia!)',
      },
      {
        trigger: 'after_dtx',
        scenario_th: 'DTX = 42 mg/dL → Hypoglycemia!\nให้ 50% Glucose 50 mL IV push\n10 นาทีต่อมา: DTX = 110 อาการอ่อนแรงหายสนิท พูดชัดปกติ → Stroke mimic!',
        correctActions: ['nihss'],
        hint_th: 'อาการหายหลังแก้น้ำตาล → ประเมิน NIHSS ซ้ำเพื่อ document ว่า deficit หายจริง (NIHSS = 0)',
      },
      {
        trigger: 'after_nihss',
        scenario_th: 'NIHSS = 0 (อาการหายสนิท)\nยังควร CT Brain confirm ว่าไม่มี structural lesion ซ่อนอยู่',
        correctActions: ['ct_brain'],
        hint_th: 'CT Brain: ปกติ ไม่มี infarct/hemorrhage → บันทึกผล CT',
      },
      {
        trigger: 'after_ct',
        scenario_th: 'CT ปกติ → สรุป: Hypoglycemia-induced focal deficit (stroke mimic)\nไม่ให้ tPA → Medical management\nวางแผนปรับยา Glibenclamide + ให้ความรู้ป้องกัน hypoglycemia',
        correctActions: ['monitor'],
        hint_th: 'เลือก "Not Eligible → Medical Rx" → Monitor ต่อเนื่อง (Continuous monitoring) + เช็ค DTX ซ้ำ',
      },
    ],
  },

  {
    ...
    steps: [
      ...
      {
        trigger: 'after_ct',
        scenario_th: 'NIHSS = 22 (Severe)\nCT Brain: ไม่มี hemorrhage (ASPECTS 9) + CTA: M1 occlusion ฝั่งขวา → LVO ยืนยัน\nOn Warfarin + INR pending + onset ไม่ทราบแน่ชัด (>4.5hr จาก last known well)',
        correctActions: ['tpa_criteria'],
        hint_th: 'ทบทวน tPA criteria → ไม่เข้าเกณฑ์ (นอก window + on Warfarin) → เลือก "Not Eligible" — แต่ LVO + ASPECTS ดี → ส่ง Mechanical thrombectomy (เกณฑ์ขยายได้ถึง 24hr)',
      },
      {
        trigger: 'after_tpa_excluded',
        scenario_th: 'ไม่ให้ tPA → Activate ทีม Interventional neuroradiology\nThrombectomy สำเร็จ TICI 3 (complete reperfusion)\nหลังทำ: อาการดีขึ้นชัดเจน NIHSS ลดเหลือ 6',
        correctActions: ['monitor'],
        hint_th: 'Post-thrombectomy → Monitor BP เข้มงวด (เป้า <180/105) + Neuro check q15-30min + ICU ระวัง reperfusion hemorrhage',
      },
    ],
  },
